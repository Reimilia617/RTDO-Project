...
fn print_usage() {
    println!("rtdo {} — Root Task Do：交互式、环境自适应的提权工具（替代 sudo）", VERSION);
    println!();
    println!("用法:");
    println!("  rtdo [选项] <命令> [参数...]   以 root 身份执行命令（自动拦截 / 交互确认）");
    println!("  rtdo --force <命令> [参数...]  强制放行，跳过所有拦截（仍需认证）");
    println!("  rtdo --policy                  查看当前策略");
    println!("  rtdo --init                    生成默认配置文件（需要 root）");
    println!("  rtdo --help                    显示帮助");
    println!("  rtdo --version                 显示版本");
    println!();
    println!("选项:");
    println!("  --force          跳过拦截（黑名单与确认），仍需认证");
    println!("  --config <路径>  指定配置文件（默认 {}/rtdo.conf）", config::DEFAULT_CONFIG_DIR);
    println!("  -h, --help       显示帮助");
...
fn parse_args(raw: &[String]) -> Result<Args, String> {
        ...
        if !in_command {
            match a.as_str() {
                ...
                "--config" => {
                    i += 1;
                    if i >= raw.len() {
                        return Err("--config 后缺少路径参数".to_string());
                    }
    ...
    let action = match action {
        Some(a) => {
            if !command.is_empty() {
                return Err("--init / --policy 不能与要执行的命令同时使用".to_string());
            }
            a
        }
        None => {
            if command.is_empty() {
                return Err("缺少要执行的命令".to_string());
            }
            Action::Run(command)
        }
    };
...
fn run_command(argv: &[String], force: bool, config_path: Option<&str>) -> ExitCode {
    ...
    if !exec::is_root() {
        eprintln!("rtdo: 必须以 root 权限运行。请先安装为 setuid root:");
    ...
    if verdict == policy::Verdict::BlockBlacklist {
        interact::notify_blocked(&cmd_display, &args_display, "命中高危命令黑名单");
        audit::log(
        ...
        loop {
            ...
            match pam::verify_root_password(&password) {
                ...
                Err(e) => {
                    ...
                    eprintln!("rtdo: {}（剩余 {} 次机会）", e, attempts);
                }
            }
        }
    }
